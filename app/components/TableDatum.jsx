    ...
    return <div style={this.props.style} className="table-datum">
      {
        ...
        .map((header) => {
          return <div data-toggle="tooltip" data-placement="top" title={`${header} - ${this.props.report[header]}`} >
            <svg id={`td${this.props.report['Department']}${header.split(" ").join("")}`} height={`${this.props.svgDimensions.height}`} width={`${this.props.svgDimensions.height}`}>
                <rect height={`${this.props.svgDimensions.height}`} width={`${this.props.svgDimensions.height}`} fill={this.props.report[header] == "Yes" ? "#8BDD3A" : (this.props.report[header] == "No" ? "#DD3D3A" : "#39BEFA") } />
            </svg>
          </div>
        })
      }
